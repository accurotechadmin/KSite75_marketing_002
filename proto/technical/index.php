...
$csrf = csrf_token();
render_header('Just One KISS — Technical Overview', 'The technical approach is built around QLC+/DMX control, projection, fog during the show, strobe-style lighting during the show, and emergency states managed by one show operator.');
render_notice($formResult);
?>
<main id="main">
  <section class="subhero section-shell" aria-labelledby="page-title">
    <p class="eyebrow">Location-facing baseline</p>
    <h1 id="page-title">Technical Overview</h1>
    <p class="hero__lede">The technical approach is built around QLC+/DMX control, projection, fog during the show, strobe-style lighting during the show, and emergency states managed by one show operator.</p>
  ...
  <section class="section-shell spectacle-section"><h2>Operating baseline</h2><div class="system-grid"><article><h3>QLC+ / DMX</h3><p>Single-universe QLC+/DMX show-control baseline for the pavilion lighting setup.</p></article><article><h3>Location-dependent effects</h3><p>Fog, bright lights, loud sound, and flashing patterns are part of the show environment; sensitive guests should plan accordingly.</p></article><article><h3>Emergency states</h3><p>Blackout, safe work light, projection black or hold, fog off, strobes off, music stop, operator reset, and performer safe look.</p></article></div></section>
</main>
<?php render_footer(); ?>
